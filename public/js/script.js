(function ($) {
	...
	$(document).ready(function() {
		$('.ads-tabs-list li').on('click', function() {
			var tabId = $(this).index();
			
			// Update active state of tabs
			$('.ads-tabs-list li').removeClass('active');
			$(this).addClass('active');
			
			// Show corresponding content
			$('.ads-tab').removeClass('active');
			$('.ads-tab').eq(tabId).addClass('active');
		});
	});

	document.addEventListener('DOMContentLoaded', function() {
		var starsContainer = document.querySelector('.twinkling-stars-bg');
		if (starsContainer) {
			var numStars = 30;
